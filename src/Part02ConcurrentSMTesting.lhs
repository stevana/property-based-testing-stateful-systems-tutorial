...
In the [previous part](./Part01SMTesting.md#readme) we saw how to test if a
sequential (single-threaded) program respects some state machine specification.
...
Next let us develop the code that does the concurrent execution, history
collection and linearisability checking.
...
2. Can you figure out ways to improve the shrinking? (Hint: see parallel
   shrinking in
   [`quickcheck-state-machine`](https://hackage.haskell.org/package/quickcheck-state-machine).)

3. How can you test that the shrinking is good/optimal? (Hint: see how
...
4. Display counterexample in a way that makes it easier to see what went wrong.
...
7. Implement a checker an other consistency model (hint: see what Jepsen's other
   checkers do).
...
- [*Finding Race Conditions in Erlang with QuickCheck and
  PULSE*](http://www.cse.chalmers.se/~nicsma/papers/finding-race-conditions.pdf) (2009)
  ([video](https://vimeo.com/6638041)) -- this paper describes how
...
testing for free via linearisability.

It's enough to find a single sequential interleaving through a concurrent
history that respects the sequential state machine model for the concurrent
execution to be correct with respect to linearisability.
...
from this part, to ensure that they are faithful to the real implementation.
